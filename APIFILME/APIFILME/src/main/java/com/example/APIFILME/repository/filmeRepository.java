...
public interface filmeRepository extends JpaRepository<filmeModel, Integer> {
    ...
    @Query(value = "SELECT * FROM ADRI_FILMES_BCK WHERE DATA_LANCAMENTO = ?1 ", nativeQuery = true)
    public List<filmeModel> findbyDATA_LANCAMENTO(int DATA_LANCAMENTO);

    @Query(value = "SELECT * FROM ADRI_FILMES_BCK WHERE UPPER(TRIM(PAIS_ORIGEM) = UPPER(PAIS_ORIGEM) ?1", nativeQuery = true)
